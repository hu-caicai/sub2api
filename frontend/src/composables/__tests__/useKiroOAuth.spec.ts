...
describe('useKiroOAuth', () => {
  ...
  it('passes the selected provider when importing a Kiro token', async () => {
    ...
    expect(mocks.importToken).toHaveBeenCalledWith({
      token_json: '{"accessToken":"access-token"}',
      device_registration_json: '{"clientId":"client-id","clientSecret":"client-secret"}',
      provider: 'Github'
    })
    expect(tokenInfo?.provider).toBe('Github')
  })

  it('keeps the import provider optional for backend defaulting', async () => {
    mocks.importToken.mockResolvedValueOnce({
      access_token: 'access-token',
      provider: 'Google'
    })
    ...
    const kiroOAuth = useKiroOAuth()
    await kiroOAuth.importToken('{"accessToken":"access-token"}')
