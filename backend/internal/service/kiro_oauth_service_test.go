...
func TestKiroOAuthService_ImportTokenUsesRequestedProvider(t *testing.T) {
	svc := NewKiroOAuthService(nil)

	token, err := svc.ImportToken(&KiroImportTokenInput{
		TokenJSON: `{"accessToken":"access-token","authMethod":"social","provider":"Google"}`,
		Provider:  kiropkg.ProviderGithub,
	})

	require.NoError(t, err)
	require.Equal(t, kiropkg.ProviderGithub, token.Provider)
}

func TestKiroOAuthService_ImportTokenDefaultsProviderToGoogle(t *testing.T) {
	svc := NewKiroOAuthService(nil)
	...
	require.NoError(t, err)
	require.Equal(t, kiropkg.ProviderGoogle, token.Provider)
}
