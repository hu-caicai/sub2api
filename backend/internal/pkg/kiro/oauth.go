...
func ParseImportedToken(tokenJSON string, deviceRegistrationJSON string, provider string) (*TokenData, error) {
	var token TokenData
	...
	// 导入接口以顶层 provider 为准；兼容旧客户端未传值时默认使用 Google。
	token.Provider = strings.TrimSpace(provider)
	if token.Provider == "" {
		token.Provider = ProviderGoogle
	}
	if !IsValidKiroProvider(token.Provider) {
		return nil, fmt.Errorf("unsupported kiro provider: %q (must be one of Google/Github/BuilderId/Enterprise/ExternalIdp)", token.Provider)
	}
